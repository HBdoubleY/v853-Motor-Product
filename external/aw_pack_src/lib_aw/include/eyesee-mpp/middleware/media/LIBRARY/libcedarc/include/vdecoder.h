...
typedef struct VIDEOSTREAMINFO {
    ...
    int   bIsAwEncodeStreamFlag; /*enable rec/ref buf reuse, only use to aw stream*/
} VideoStreamInfo;
...
//* NO.13
int RotatePicture(MEMOPS_STRUCT *memOps,
...
VideoPicture *AllocatePictureBuffer(MEMOPS_STRUCT *memOps,
		int nWidth,
		int nHeight,
		int nLineStride,
		int ePixelFormat);
...
int FreePictureBuffer(MEMOPS_STRUCT *memOps,

...
#define LINKSDK_VERSION "V3.6.7_1017"
...
#define LINK_RESOURCE_MASK_VOICE                      (1 << 2)
#define LINK_RESOURCE_MASK_TELPHONE                   (1 << 3)
#define LINK_RESOURCE_MASK_NAVI                       (1 << 4)
#define LINK_RESOURCE_MASK_VOICE_SPEECH               (1 << 5)
#define LINK_RESOURCE_MASK_AUX_VOICESPEECH            (1 << 6)
...
/**
   ...
   * @note  json string format : {
    ...
    @note state talking:1 hold:2 idle:3 incomming:4 conferenced:5 muted:6
}
   */
...
/**
   ...
   * @param lparam [in] reverse
   * @param cparam [in] yyyyMMdd hh:mm:ss
   */
...
/**
   * @brief Media playback status change
   * @param lparam [in] json len
   * @param cparam [in] json datas
   * @note "{\"state\":%d,\"media_source\":\"%s\",\"playback_seconds\":%u,\"shuffle\":%d,\"repeat\":%d,\"repeat_one\":%d}"
   * @note state stop:1 playing:2 paused:3
   */
#define SINK_NOTIFY_MEIDAPLAYBACK_STATUS_CHANGE             0x210


/**
   * @brief Media playback metadata change
   * @param lparam [in] json len
   * @param cparam [in] json datas
   * @note "{\"song\":\"%s\",\"album\":\"%s\",\"artist\":\"%s\",\"playlist\":\"%s\",\"duration_seconds\":%u,\"rating\":%d}",
   */
#define SINK_NOTIFY_MEIDAPLAYBACK_METADATA_CHANGE           0x211

/**
   * @brief Media playback art img change
   * @param lparam [in] 0
   * @param cparam [in] img cache path
   */
#define SINK_NOTIFY_MEIDAPLAYBACK_ALBUMCOVER_CHANGE         0x212

/**
   * @brief Navigation status change
   * @param lparam [in] status
   * @param cparam [in] reverse
   * @note status values (UNAVAILABLE = 0, ACTIVE = 1, INACTIVE = 2, REROUTING = 3)
   */
#define SINK_NOTIFY_NAVIGATION_STATUS_CHANGE                 0x213

/**
   * @brief Navigation state change
   * @param lparam [in] json len
   * @param cparam [in] json datas
   * @note  {"steps":[{"maneuver":{"type":%u,"roundabout_exit_number":%d,"roundabout_exit_angle":%d},"road":"%s","lanes":[],"cue":["%s"]}],"destinations":["%s"]}
   */
#define SINK_NOTIFY_NAVIGATION_STATE_CHANGE                 0x214

/**
   * @brief Navigation current position change
   * @param lparam [in] json len
   * @param cparam [in] json datas
   * @note  {"step_distance":{"meters":%d,"display_value":"%s","display_units":%d},"time_to_step_seconds":%d,"destination_distances":[{"meters":%d,"display_value":"%s","display_units":%u,"estimated_time_at_arrival":"%s","time_to_arrival_seconds":%PRId64}],"current_road":"%s"}
   */
#define SINK_NOTIFY_NAVIGATION_POSITION_CHANGE              0x215
...
/**
   ...
   * @param lparam [in] reverse
   * @param sparam [in] latitudeE7=xxxx&longitudeE7=xxxx&accuracyE3=xx&altitudeE2=xx&speedE3=xx&bearingE6=xx
...
/**
   * @brief Report fuel datas
   * @param lparam [in] reverse
   * @param cparam [in] fuel data string params, fuelRemainingPercent=100&rangeKm=800&lowFuelWarning=0
   * @note You can choose to fill in some fields or fill in all fields
   */
#define LINK_ACTION_REPORT_FUEL_DATAS                              0x11A
...
/**
   * @brief Enable HiCar\CarLink ble broadcast
...
/**
   ...
   * @param lparam [in] reverse
   * @param cparam [in] AT command SR|SV|SS|SI
   */
...
/**
   * @brief Request audio aec process
   * @param lparam [in] 0:stop 1:start
   * @param cparam [in] reverse
   * @note for android hicar
   */
#define LINK_ACTION_HICAR_REQUEST_AEC_PROCESS                       0x12C

/**
   * @brief Get HiCar pincode
   * @param lparam [in] reverse
   * @param cparam [in] reverse
   * @note must enable hicar ble adv
   */
#define LINK_ACTION_HICAR_GET_PIN_CODE                              0x12D

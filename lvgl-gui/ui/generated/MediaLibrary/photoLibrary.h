...
#define DEFAULT_PHOTO_DIR "/mnt/extsd/DVRpic"    /* 默认照片目录 */
...
typedef struct  {
    lv_obj_t *cont;                     // 创建主容器（全屏）
    lv_obj_t *top_bar;                  // 创建顶部栏
    lv_obj_t *scan_bar;                 // 创建扫描栏

    lv_style_t style_btn;               // 按钮样式

    lv_obj_t *left_placeholder;         // 添加左侧占位控件
    lv_obj_t *title;                    // 创建标题
    lv_obj_t *refresh_btn;              // 刷新按钮
    lv_obj_t *refresh_icon;
    lv_obj_t *info_label;               // 信息标签显示找到了多少张图片
    // 空列表需要增加显示内容
    lv_obj_t *empty_label1;
    lv_obj_t *empty_label2;
    lv_obj_t *empty_label3;

    lv_obj_t *list_cont;                // 列表容器
    lv_obj_t *list;                     // 照片列表

    char **photo_paths;                 // 照片路径数组
    int current_index;                  // 当前显示的照片索引
    int photo_count;                    // 照片数量
    int photo_capacity;                 // 数组容量
    char photo_dir[256];                // 照片目录路径

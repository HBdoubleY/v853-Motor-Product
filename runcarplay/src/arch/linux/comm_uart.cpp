...
int comm_uart::open_dev(const char *dev, int *fd) {
	tv.tv_sec = 0;
    tv.tv_usec = 10000;
...
int comm_uart::close_dev(int fd) {
	return (close(fd));
}
...
int comm_uart::set_speed(int fd, speed_t speed) {
	...
	if (fd != -1) { //判断传进来是否相等
		...
		status = tcsetattr(fd, TCSANOW, &Opt); //这是立刻把baut rates设置真正写到串口中去
...
int comm_uart::read(int fd, char *data, int size) {
   ...
   {  
       if( retval &&   FD_ISSET(fd, &rfds)  )
       {
           ...
           if(rc>0)
           {
               //printf("recv serial----rc  testi is %d \n  ",rc);
               for(i = 0; i < size; i++)
               //printf("%x ", data[i]);
               //printf("\n");
               return rc;

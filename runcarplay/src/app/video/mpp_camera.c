...
#if 0
static void mallocDstFrameMem(mpp_camera_para_conf *pContext){
    printf("%s:%d\n",__func__,__LINE__);
    int ret;
    int i;
    for(i = 0; i < 3; i++){
        if(pContext->m_vo.dstFrameInfo[i] != NULL) continue;
        pContext->m_vo.dstFrameInfo[i] = (VIDEO_FRAME_INFO_S *)malloc(sizeof(VIDEO_FRAME_INFO_S));
        if( pContext->m_vo.dstFrameInfo[i] == NULL){
            return;
        }       
        pContext->m_vo.dstFrameInfo[i]->mId = 0;
        pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth = AWALIGN(pContext->m_vi.mHeight, 16);
        pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight = AWALIGN(pContext->m_vi.mWidth, 16);
        pContext->m_vo.dstFrameInfo[i]->VFrame.mPixelFormat = pContext->m_vi.mPixFmt;
        printf("dstFrameInfo->VFrame.mWidth:%d,dstFrameInfo->VFrame.mHeight:%d,dstFrameInfo->VFrame.mPixelFormat:%x\n",pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth,pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight,pContext->m_vo.dstFrameInfo[i]->VFrame.mPixelFormat);
        switch (pContext->m_vi.mPixFmt)
        {
        case MM_PIXEL_FORMAT_YUV_SEMIPLANAR_420:
        case MM_PIXEL_FORMAT_YVU_SEMIPLANAR_420:
            ret = AW_MPI_SYS_MmzAlloc_Cached(&pContext->m_vo.dstFrameInfo[i]->VFrame.mPhyAddr[0],&pContext->m_vo.dstFrameInfo[i]->VFrame.mpVirAddr[0],pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth*pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight);
            if(ret != SUCCESS){
                printf("fatal error! phy alloc fail:%d\n", ret);
                goto mem_err;
            }
            ret = AW_MPI_SYS_MmzAlloc_Cached(&pContext->m_vo.dstFrameInfo[i]->VFrame.mPhyAddr[1],&pContext->m_vo.dstFrameInfo[i]->VFrame.mpVirAddr[1],pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth*pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight/2);
            if(ret != SUCCESS){
                printf("fatal error! phy alloc fail:%d\n", ret);
                goto mem_err;
            }    
        ...
        case MM_PIXEL_FORMAT_YVU_PLANAR_420:  
            ...
            ret = AW_MPI_SYS_MmzAlloc_Cached(&pContext->m_vo.dstFrameInfo[i]->VFrame.mPhyAddr[1],&pContext->m_vo.dstFrameInfo[i]->VFrame.mpVirAddr[1],pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth*pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight/4);
            if(ret != SUCCESS){
                printf("fatal error! phy alloc fail:%d\n", ret);
                goto mem_err;
            }
            ret = AW_MPI_SYS_MmzAlloc_Cached(&pContext->m_vo.dstFrameInfo[i]->VFrame.mPhyAddr[2],&pContext->m_vo.dstFrameInfo[i]->VFrame.mpVirAddr[2],pContext->m_vo.dstFrameInfo[i]->VFrame.mWidth*pContext->m_vo.dstFrameInfo[i]->VFrame.mHeight/4);
            if(ret != SUCCESS){
                printf("fatal error! phy alloc fail:%d\n", ret);
                goto mem_err;
            }
    ...
    pContext->m_vo.readyFrameNum = 0;
    printf("AW_MPI_SYS_MmzAlloc_Cached end!!!\n");
mem_err:
    for(int o = 0; o < i; o++){
        free(pContext->m_vo.dstFrameInfo[o]);
    }
    return;  
}
#endif
...
#if 0
void *Vi2VoFrameThread(void *pThreadData){
    mpp_camera_para_conf *pContext = (mpp_camera_para_conf*)pThreadData;
    VIDEO_FRAME_INFO_S *dstFrameInfo = NULL;
    VIDEO_FRAME_INFO_S *stFrameInfo = NULL;
    int ret = 0;
    dstFrameInfo = (VIDEO_FRAME_INFO_S *)malloc(sizeof(VIDEO_FRAME_INFO_S));
    if(dstFrameInfo == NULL){
        return;
    }
    stFrameInfo = (VIDEO_FRAME_INFO_S *)malloc(sizeof(VIDEO_FRAME_INFO_S));
    if(stFrameInfo == NULL){
        free(dstFrameInfo);
        return;
    }     
    memset(dstFrameInfo, 0, sizeof(VIDEO_FRAME_INFO_S));  
    ...
    printf("pContext->m_vi.mHeight:%d,pContext->m_vi.mWidth:%d,pContext->m_vi.mPixFmt:%x\n",pContext->m_vi.mHeight,pContext->m_vi.mWidth,pContext->m_vi.mPixFmt);
    ...
    {  
        if(pContext->mVoFlag){
            ...
            {
                printf("-------mppQueue is empty-----\n");
                usleep(40000);
                continue;
            } 
            stFrameInfo = (VIDEO_FRAME_INFO_S *)queue_mpp_pop(pContext->m_vo.mppQueue);
...
mem_err:
    AW_MPI_SYS_MmzFree(dstFrameInfo->VFrame.mPhyAddr[2],dstFrameInfo->VFrame.mpVirAddr[0]);
    AW_MPI_SYS_MmzFree(dstFrameInfo->VFrame.mPhyAddr[2],dstFrameInfo->VFrame.mpVirAddr[1]);
    AW_MPI_SYS_MmzFree(dstFrameInfo->VFrame.mPhyAddr[2],dstFrameInfo->VFrame.mpVirAddr[2]);
    free(dstFrameInfo);
}
#endif
...
void *Vi2VoFrameThread(void *pThreadData)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        dstBuffers[i]->mId = 0;
        // 注意：G2D旋转后宽高对调
        dstBuffers[i]->VFrame.mWidth = AWALIGN(pContext->m_vi.mHeight, 16);
        dstBuffers[i]->VFrame.mHeight = AWALIGN(pContext->m_vi.mWidth, 16);
        ...
        case MM_PIXEL_FORMAT_YVU_PLANAR_420:
            ...
            ret = AW_MPI_SYS_MmzAlloc_Cached(
                &dstBuffers[i]->VFrame.mPhyAddr[1],
                &dstBuffers[i]->VFrame.mpVirAddr[1],
                dstBuffers[i]->VFrame.mWidth * dstBuffers[i]->VFrame.mHeight / 4);
            if (ret != SUCCESS) {
                printf("Buffer %d plane1 alloc failed: %d\n", i, ret);
                if (dstBuffers[i]->VFrame.mpVirAddr[0]) {
                    AW_MPI_SYS_MmzFree(dstBuffers[i]->VFrame.mPhyAddr[0], 
            ...
            ret = AW_MPI_SYS_MmzAlloc_Cached(
                &dstBuffers[i]->VFrame.mPhyAddr[2],
                &dstBuffers[i]->VFrame.mpVirAddr[2],
                dstBuffers[i]->VFrame.mWidth * dstBuffers[i]->VFrame.mHeight / 4);
            if (ret != SUCCESS) {
                printf("Buffer %d plane2 alloc failed: %d\n", i, ret);
                if (dstBuffers[i]->VFrame.mpVirAddr[0]) {
                    AW_MPI_SYS_MmzFree(dstBuffers[i]->VFrame.mPhyAddr[0], 
                                      dstBuffers[i]->VFrame.mpVirAddr[0]);
                }
    ...
    while (pContext->mExitFlag == 0)
    {
        ...
        if (pContext->mVoFlag) {
            ...
            // 检查源帧和目标帧的格式是否匹配
            if (stFrameInfo->VFrame.mPixelFormat != dstBuffers[currentBuffer]->VFrame.mPixelFormat) {
                printf("Pixel format mismatch! src:%x, dst:%x\n", 
                       stFrameInfo->VFrame.mPixelFormat,
                       dstBuffers[currentBuffer]->VFrame.mPixelFormat);
                free(stFrameInfo);
                continue;
            }
            
            // G2D旋转
            ret = G2D_Convert_rotate(stFrameInfo, dstBuffers[currentBuffer], 270);
            if (ret != 0) {
            ...
            char str[30];
            static int num = 0;
            sprintf(str, "/mnt/extsd/DVRpic/pic%d.yuv", num);
            num ++;
            FILE *fp = fopen(str, "wb");
            if(fp)
            {
                int write_len = dstBuffers[currentBuffer]->VFrame.mWidth * dstBuffers[currentBuffer]->VFrame.mHeight;
                AW_MPI_SYS_MmzFlushCache(dstBuffers[currentBuffer]->VFrame.mPhyAddr, dstBuffers[currentBuffer]->VFrame.mpVirAddr[0], write_len);
                AW_MPI_SYS_MmzFlushCache(dstBuffers[currentBuffer]->VFrame.mPhyAddr, dstBuffers[currentBuffer]->VFrame.mpVirAddr[1], write_len/2);
                fwrite(dstBuffers[currentBuffer]->VFrame.mpVirAddr[0], write_len, 1, fp);
                fwrite(dstBuffers[currentBuffer]->VFrame.mpVirAddr[1], write_len/2, 1, fp);
                fclose(fp);
            }
#endif

            // 发送旋转后的帧到VO
            ret = AW_MPI_VO_SendFrame(pContext->m_vo.mVoLayer, 
                                     pContext->m_vo.mVoChn, 
                                     dstBuffers[currentBuffer], 0);
            if (ret < 0) {
                printf("VO send frame failed: %d\n", ret);
            }
...
void *GetCSIFrameThread(void *pThreadData)
{
    ...
    // VIDEO_FRAME_INFO_S *framePool[MAX_QUEUE_FRAMES];
    // int poolIndex = 0;
    
    // // 初始化帧池
    // for (int i = 0; i < MAX_QUEUE_FRAMES; i++) {
    //     framePool[i] = (VIDEO_FRAME_INFO_S *)calloc(1, sizeof(VIDEO_FRAME_INFO_S));
    //     if (!framePool[i]) {
    //         printf("Failed to allocate frame pool buffer %d\n", i);
    //         // 清理已分配的
    //         for (int j = 0; j < i; j++) {
    //             free(framePool[j]);
    //         }
    ...
    {      
        ...
        if(pContext->mVoFlag){
            VIDEO_FRAME_INFO_S *dstFrame = (VIDEO_FRAME_INFO_S *)malloc(sizeof(VIDEO_FRAME_INFO_S));
            memset(dstFrame, 0, sizeof(VIDEO_FRAME_INFO_S));
            memcpy(dstFrame, &stFrameInfo, sizeof(VIDEO_FRAME_INFO_S));
            
            // 将缓冲区推入队列
            queue_mpp_push(pContext->m_vo.mppQueue, dstFrame);
            
            // 移动到下一个缓冲区
            // poolIndex = (poolIndex + 1) % MAX_QUEUE_FRAMES;
        }
...
ERRORTYPE createViChn(mpp_camera_para_conf *pContext, VI_DEV videv, VI_CHN vichn){
    ...
    ret = pthread_create(&pContext->mVi2VoThreadId, NULL, Vi2VoFrameThread, pContext);
    if (ret != 0)
    {
        printf("fatal error! create Vi2VoFrameThread Thread fail[%d]\n", ret);
        return ret;
    }
    else
    {
        printf("create Vi2VoFrameThread Thread success! threadId[0x%x]\n", &pContext->mVi2VoThreadId);
    }
...
ERRORTYPE stopVideoRecording(mpp_camera_para_conf *pContext){
    ...
    if (pContext->m_venc.mVEncChn >= 0 && pContext->m_mux.mMuxChn >= 0)
    {
        ...
        printf("AW_MPI_SYS_UnBind(&VeChn, &MuxChn)\n");
    }
    if(pContext->m_ai.mAiChn.mChnId >= 0 && pContext->m_aenc.mAEncChn.mChnId >= 0 && pContext->m_mux.mMuxChn >= 0){  
        MPP_CHN_S MuxChn = {MOD_ID_MUX, 0, pContext->m_mux.mMuxChn};
        AW_MPI_SYS_UnBind(&pContext->m_aenc.mAEncChn, &MuxChn);
        printf("AW_MPI_SYS_UnBind(&mAEncChn, &MuxChn)\n");
        ...
        printf("AW_MPI_SYS_UnBind(&mAiChn, &mAEncChn)\n");
    }

...
status_t SamplePlayerContext::loadConfig()
{
    ...
    mConfigPara.mfVps = (float)GetConfParaDouble(&stConfParser, SAMPLE_PLAYER_KEY_VPS, 1);
    mConfigPara.mAudioVolume = (float)GetConfParaDouble(&stConfParser, SAMPLE_PLAYER_KEY_AUDIO_VOLUME, 1);
    destroyConfParser(&stConfParser);
    return SUCCESS;
}

int MP4_Palyer_test_main()
{
    int result = 0;
    ...
    // hyby test start
    int argc = 3;
    char *argv[] = {"hyby_test_main", "-path", "/mnt/extsd/sample_Player.conf", NULL};
    // hyby test end
    ...
    pContext->mpPlayer->setVps(pContext->mConfigPara.mfVps);
    pContext->mpPlayer->prepare();
    pContext->mpPlayer->setVolume(pContext->mConfigPara.mAudioVolume, pContext->mConfigPara.mAudioVolume);
...
//    }
    ...
    /*{
        ...
        alogd("restore vps[%f]", pContext->mConfigPara.mfVps);
        pContext->mpPlayer->setVps(pContext->mConfigPara.mfVps);
    }*/
